...
class GDI
  class Debugger
    ...
    class << self
      ...
      def active_debugger_for_title title
        active_debuggers.detect {|d| title =~ d.src_extensions }
      end
      ...
      # An array with supported action definitions (as hashes) for use as buttons
      # Inherited
      def supported_actions(*actions)
        actions.any? ? @actions = actions : get_inherited_attribute("@actions")
      end
      ...
      # Test if this debugger can work without an open project in the current
      # window
      def require_project?
        !!get_inherited_attribute("@require_project")
      end
    ...
    def delete_breakpoint(breakpoint)
      @process.input("#{self.class::ClearBreak} #{breakpoint.file}:#{breakpoint.line}")
    end
