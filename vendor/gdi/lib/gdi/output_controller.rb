...
class GDI
  class  OutputController
    ...
    def initialize(process_controller, title = nil)
      @process_controller = process_controller
      @title = title
      process_controller.add_listener(:run) { show_tab }
      process_controller.add_listener(:prompt_ready) {
        status("Ready")
        execute(<<-JAVASCRIPT)
        $(".action").removeAttr("disabled");
        JAVASCRIPT
      }
      process_controller.add_listener(:prompt_blocked) {
        status("Blocked")
        execute(<<-JAVASCRIPT)
    ...
    def index
      ...
      @actions = model.class.supported_actions
      render("index")
    end
    ...
    def command(command)
      @process_controller.input(command)
    end
    ...
    def show_tab
      unless @tab
        Redcar.app.focussed_window.tap do |w|
          ...
          if notebook = w.notebooks.detect {|n| n.tabs.detect {|t| t.is_a? GDI::Tab }}
            w.set_focussed_notebook notebook
          else
            if w.notebooks.size == 1
              w.create_notebook
              w.notebook_orientation = :vertical
            elsif w.notebooks.last.tabs.size > 0
              w.create_notebook
            end
            w.set_focussed_notebook w.notebooks.last
          end
          @tab = w.new_tab(GDI::Tab)
    ...
    def title
      @title ||= "GDI: #{@process_controller.commandline}"
    end
  end
end
