...
class GDI::ProcessController
  ...
  def initialize(options)
    @output      = GDI::OutputController.new(self, options[:title])
    @model       = options[:model].new(@output, self)
    @connection  = options[:connection]
    @path        = options[:directory].length > 0 ? options[:directory] : nil
  ...
  def commandline
    cmd = "#{@model.class::Commandline} #{@connection} #{@arguments}".strip
    if @path
      "sh -c \"cd #{@path}; #{cmd}\""
    else
      cmd
  ...
  def run_posix
    _, @stdin, @stdout, stderr = IO.popen4("#{commandline}")
